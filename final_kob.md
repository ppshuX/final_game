...
- **开发状态**：🚀 进行中
- **当前阶段**：Day 2 - 用户注册/登录模块开发
- **目标状态**：腾讯面试可展示级后端项目 ✅

## 每日进度更新

### 2025年10月21日（周一）- Day 1 已完成 ✅
- 📌 **当前状态**：Day 1开发阶段完成，项目基础架构搭建完成
- 🎯 **今日目标**：完成项目初始化 + Git 配置 + 数据库设计 + 项目结构创建
- ✅ **已完成**：
  - ✅ **Lesson 2: 创建项目结构 backend,web,acapp**（2025/10/21 10:55）
- 💭 **技术成果**：
  - ✅ **Spring Boot 项目初始化**
  - ✅ **Maven 项目配置**
  - ✅ **Git 仓库配置**
...
- 💡 **技术说明**：
  ...
  - Maven 依赖管理配置
  - Git 版本控制集成
  - 三端分离架构设计（Backend + Web + AcApp）
  - 基础配置文件创建
  - 项目目录结构标准化
- 📁 **主要文件创建**：
  - **Backend 模块（Spring Boot）**：
    - `backend/pom.xml` - Maven 项目配置
    - `backend/src/main/java/com/final_kob/backend/BackendApplication.java` - Spring Boot 主类
    - `backend/src/main/java/BasicController.java` - 基础控制器
    - `backend/src/main/java/PathVariableController.java` - 路径变量控制器
    - `backend/src/main/java/User.java` - 用户实体类
    - `backend/src/main/java/com/final_kob/backend/config/CorsConfig.java` - 跨域配置
    - `backend/src/main/java/com/final_kob/backend/controller/pk/IndexController.java` - PK 控制器
    - `backend/src/main/java/com/final_kob/backend/controller/pk/BotInfoController.java` - Bot 信息控制器
    - `backend/src/main/resources/application.properties` - 应用配置文件
    - `backend/src/main/resources/static/index.html` - 静态首页
    - `backend/src/main/resources/templates/pk/index.html` - PK 页面模板
  - **Web 模块（Vue3）**：
    - `web/package.json` - npm 项目配置
    - `web/src/App.vue` - Vue 根组件
    - `web/src/main.js` - Vue 入口文件
    - `web/src/router/index.js` - Vue Router 配置
    - `web/src/store/index.js` - Vuex 状态管理
    - `web/src/components/Helloworld.vue` - Hello World 组件
    - `web/vue.config.js` - Vue CLI 配置
  - **AcApp 模块（Vue3）**：
    - `acapp/package.json` - npm 项目配置
    - `acapp/src/App.vue` - Vue 根组件
    - `acapp/src/main.js` - Vue 入口文件
    - `acapp/src/store/index.js` - Vuex 状态管理
    - `acapp/src/components/Helloworld.vue` - Hello World 组件
    - `acapp/vue.config.js` - Vue CLI 配置
- ⏰ **更新时间**：2025/10/21 10:55
...
| 2025/10/22 | ⚙️ | 实现 JWT 登录注册模块 | - |
| 2025/10/23 | 🧩 | 完成个人中心模块 | - |
| 2025/10/24 | 🎯 | 匹配系统开发 | - |
| 2025/10/25 | 🤖 | Bot 执行系统上线 | - |
| 2025/10/26 | 🏆 | 排行榜模块完成 | - |
| 2025/10/27 | ✅ | 项目打包上线 | - |
